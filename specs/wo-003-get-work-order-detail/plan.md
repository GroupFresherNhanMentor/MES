...
### Domain & Port Layer
- [MODIFY] `be/src/main/java/fpt/qn/mes/workorder/domain/repository/WorkOrderRepository.java`: Add unpaginated list methods `findMaterialsByWorkOrderId(UUID workOrderId)` and `findEventsByWorkOrderId(UUID workOrderId)`.
...
- [MODIFY] `be/src/main/java/fpt/qn/mes/workorder/application/service/WorkOrderService.java`: Implement `getWorkOrderById(UUID id)` fetching WorkOrder, materials, and events, mapping to `WorkOrderDto`.

### Persistence Layer
- [MODIFY] `be/src/main/java/fpt/qn/mes/workorder/infrastructure/persistence/WorkOrderRecordMapper.java`: Implement `toDomain(WorkOrderMaterialsRecord r)` and `toDomain(WorkOrderEventsRecord r)`.
