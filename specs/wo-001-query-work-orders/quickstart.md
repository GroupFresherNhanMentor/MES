# Quickstart Validation Guide: Query Work Orders (`GET /api/v1/work-orders`)

## Verification Overview

This guide provides end-to-end instructions for validating the `GET /api/v1/work-orders` API endpoint.
...
```bash
curl -X GET "http://localhost:8080/api/v1/work-orders?page=0&size=20" \
...
```bash
curl -X GET "http://localhost:8080/api/v1/work-orders?finishedProductId=a1b2c3d4-e5f6-7a8b-9c0d-1e2f3a4b5c6d&statusId=c3d4e5f6-a7b8-9c0d-1e2f-3a4b5c6d7e8f" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X GET "http://localhost:8080/api/v1/work-orders?code=0001" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X GET "http://localhost:8080/api/v1/work-orders"
```
