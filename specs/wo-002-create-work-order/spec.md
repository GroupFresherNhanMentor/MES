# Feature Specification: Create Work Order (`POST /api/work-orders`)
...
**Independent Test**: Can be tested independently by issuing a `POST /api/work-orders` request with valid payload. Verify that the Work Order record is created with `bomId` populated from the active BOM and `work_order_materials` records are calculated based on `plannedQuantity * quantityPerUnit * (1 + scrapRate)`.
