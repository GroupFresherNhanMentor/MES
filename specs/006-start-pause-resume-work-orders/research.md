...
### Decision
- `POST /api/work-orders/{id}/start` -> Accepts `StartWorkOrderRequest` body with required `machineId` and optional `productionLineId` and `operatorId`. Returns `ApiResponse<WorkOrderDto>`.
- `POST /api/work-orders/{id}/pause` -> No request body. Returns `ApiResponse<WorkOrderDto>`.
- `POST /api/work-orders/{id}/resume` -> No request body. Returns `ApiResponse<WorkOrderDto>`.
