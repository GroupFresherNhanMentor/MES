package fpt.qn.mes.master.line.infrastructure.seed;
...
public class LineMasterDataSeeder implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        List<Map<String, Object>> rows = loadJson("line-statuses.json");
        if (rows.isEmpty()) return;

        var step = ctx.insertInto(LINE_STATUSES, LINE_STATUSES.ID, LINE_STATUSES.NAME, LINE_STATUSES.DESCRIPTION);
        for (Map<String, Object> row : rows) {
            step = step.values(UUID.randomUUID(), (String) row.get("name"), (String) row.get("description"));
        }
        step.onConflictDoNothing().execute();
        log.info("Seeded line_statuses");
    }
