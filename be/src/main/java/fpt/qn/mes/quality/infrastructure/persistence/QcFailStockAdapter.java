...
public class QcFailStockAdapter implements QcFailStockPort {

    DSLContext ctx;

    @Override
    public void scrapStock(UUID lotId, UUID productId, BigDecimal quantity, UUID workOrderId, UUID inspectorId) {
        transfer(lotId, productId, quantity, workOrderId, inspectorId,
                StockStatusConstants.SCRAPPED, MovementTypeConstants.SCRAP);
    }

    @Override
    public void holdStock(UUID lotId, UUID productId, BigDecimal quantity, UUID workOrderId, UUID inspectorId) {
        transfer(lotId, productId, quantity, workOrderId, inspectorId,
                StockStatusConstants.ON_HOLD, MovementTypeConstants.QC_HOLD);
    }

    private void transfer(UUID lotId, UUID productId, BigDecimal quantity,
            UUID workOrderId, UUID inspectorId,
            String toStatusName, String movementTypeName) {
        UUID fromStatusId   = resolveStatusId(StockStatusConstants.QUALITY_INSPECTION);
        UUID toStatusId     = resolveStatusId(toStatusName);
        UUID movementTypeId = resolveMovementTypeId(movementTypeName);
        ...
        var target = ctx.selectFrom(STOCK_BALANCES)
            .where(STOCK_BALANCES.LOT_ID.eq(lotId))
            .and(STOCK_BALANCES.PRODUCT_ID.eq(productId))
        ...
        if (target != null) {
            ctx.update(STOCK_BALANCES)
                ...
                .set(STOCK_BALANCES.UPDATED_AT, OffsetDateTime.now())
                .where(STOCK_BALANCES.ID.eq(target.getId()))
                .execute();
        } else {
