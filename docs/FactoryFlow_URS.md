...
- Cấp phát/giữ nguyên vật liệu cho sản xuất.
...
**Acceptance criteria:**
- Given user nhập đúng username/password
  When gọi API login
  Then hệ thống trả về JWT token hợp lệ
- Given user nhập sai password
  When gọi API login
  Then hệ thống trả về lỗi authentication failed
...
**Acceptance criteria:**
- Given user không có quyền
  When gọi API bị giới hạn
...
**Acceptance criteria:**
- Given Admin tạo product với code chưa tồn tại
  Then hệ thống tạo product thành công
- Given Admin tạo product với code đã tồn tại
  Then hệ thống trả lỗi duplicate product code
...
**Ví dụ:** `WH-HN / A01`, `WH-HN / A02`, `WH-HCM / B01`
...
**Machine status:**
- AVAILABLE, RUNNING, DOWN, UNDER_MAINTENANCE, RETIRED
...
**Stock status:**
- AVAILABLE, RESERVED, ON_HOLD, QUALITY_INSPECTION, DAMAGED, SCRAPPED, CONSUMED, SHIPPED
...
**Acceptance criteria:**
- Given stock balance quantity = 10
  When hệ thống thực hiện movement xuất 3
  Then quantity còn 7
- Given stock balance quantity = 2
  When hệ thống xuất 5
  Then hệ thống reject vì không đủ tồn kho
...
**Acceptance criteria:**
- Given Warehouse Manager nhập 100 nguyên vật liệu A vào WH1/A01
  Then stock balance AVAILABLE tăng 100
  And stock movement PURCHASE_IN được tạo
...
**Acceptance criteria:**
- Given stock balance = 100
  When adjustment -10 với reason hợp lệ
  Then stock balance = 90
  And movement ADJUSTMENT được tạo
...
**Acceptance criteria:**
- Given WH1/A01 có 50 item A
  When transfer 20 item A sang WH1/A02
  Then WH1/A01 còn 30
  And WH1/A02 tăng 20
  And có movement TRANSFER_OUT, TRANSFER_IN
...
**Movement type:**
- PURCHASE_IN, TRANSFER_IN, TRANSFER_OUT, RESERVE, RELEASE_RESERVATION, ISSUE_TO_PRODUCTION, CONSUME_IN_PRODUCTION, PRODUCTION_OUTPUT, QC_HOLD, QC_RELEASE, SCRAP, ADJUSTMENT, SHIP_OUT, RETURN_TO_WAREHOUSE
...
**Acceptance criteria:**
- Given hệ thống reserve material
  Then stock movement type RESERVE được tạo
- Given user cố update stock movement đã tạo
...
**Acceptance criteria:**
- Given Planner tạo BOM cho Product FG-001
  And BOM gồm Material A quantity 2, Material B quantity 1
  Then hệ thống tạo BOM thành công
...
**Acceptance criteria:**
- Given BOM version 1 đã được dùng trong Work Order
  When Planner update quantity của BOM item
  Then hệ thống reject và yêu cầu tạo version mới
...
**Work Order status:**
- DRAFT, PLANNED, MATERIAL_RESERVED, MATERIAL_SHORTAGE, READY_TO_PRODUCE, IN_PROGRESS, PAUSED, COMPLETED, CANCELLED, QC_HOLD
...
**Acceptance criteria:**
- Given Planner tạo WO sản xuất 100 Product FG-001
  And FG-001 có BOM active
  Then Work Order được tạo thành công
...
**Acceptance criteria:**
- Given BOM của FG-001 cần 2 Material A cho 1 sản phẩm
  When tạo WO sản xuất 100 FG-001
  Then hệ thống tính required Material A = 200
...
**Acceptance criteria:**
- Given WO đang COMPLETED
  When user cancel WO
  Then hệ thống reject vì transition không hợp lệ
...
**Acceptance criteria:**
- Given Material A available = 500
  And WO cần Material A = 200
  When Planner reserve material
  Then available giảm 200
  And reserved tăng 200
  And WO status = MATERIAL_RESERVED
  And movement RESERVE được tạo

**Acceptance criteria thiếu vật tư:**
- Given Material A available = 100
  And WO cần Material A = 200
  When Planner reserve material
  Then hệ thống không reserve
  And WO status = MATERIAL_SHORTAGE
  And trả lỗi thiếu Material A 100 units
...
**Acceptance criteria:**
- Given WO đã reserve 200 Material A
  When Planner cancel WO
  Then reserved giảm 200
  And available tăng 200
  And movement RELEASE_RESERVATION được tạo
...
- Given Material A available = 10
  ...
  Then chỉ 10 request success
  And 10 request failed
  And available = 0
  And reserved = 10
...
**Acceptance criteria:**
- Given WO đã reserve đủ material
  And Machine M1 AVAILABLE
  When Operator start production
  Then WO status = IN_PROGRESS
  And Machine M1 status = RUNNING
  And production event START được tạo

**Acceptance criteria machine lỗi:**
- Given Machine M1 UNDER_MAINTENANCE
  When Operator start production bằng M1
  Then hệ thống reject
...
**Acceptance criteria:**
- Given WO sản xuất 100 sản phẩm
  When Operator complete với actual=100, good=95, defect=5, scrap=0
  Then WO status = COMPLETED
  And nguyên vật liệu được consume
  And thành phẩm được tạo ở trạng thái QUALITY_INSPECTION
  And QC inspection được tạo
  And movement PRODUCTION_OUTPUT được tạo
...
**QC status:**
- PENDING_INSPECTION, PASSED, FAILED, ON_HOLD, REWORK_REQUIRED, SCRAPPED
...
**Acceptance criteria:**
- Given 100 finished goods đang QUALITY_INSPECTION
  When QC pass 95
  Then 95 chuyển AVAILABLE
  And movement QC_RELEASE được tạo
...
**Acceptance criteria:**
- Given QC fail 5 sản phẩm với action SCRAP
  Then 5 sản phẩm chuyển SCRAPPED
  And movement SCRAP được tạo
  And defect reason được lưu
...
**Acceptance criteria:**
- Given Machine M1 UNDER_MAINTENANCE
  When Maintenance Engineer close ticket thành công
  Then ticket status = CLOSED
  And Machine M1 status = AVAILABLE
  And downtime được ghi nhận
...
- CREATE_WORK_ORDER, RESERVE_MATERIAL, RELEASE_RESERVATION, START_PRODUCTION, PAUSE_PRODUCTION, RESUME_PRODUCTION, COMPLETE_PRODUCTION, QC_PASS, QC_FAIL, QC_HOLD, QC_RELEASE, SCRAP_STOCK, CREATE_MAINTENANCE_TICKET, START_MAINTENANCE, CLOSE_MAINTENANCE_TICKET, ADJUST_STOCK, ACTIVATE_BOM
...
- stock_lots, stock_balances, stock_movements
- boms, bom_items
- work_orders, work_order_materials, work_order_events
- production_lines, machines, production_runs
- quality_inspections, quality_inspection_results, defect_types
- maintenance_tickets, machine_downtimes
...
- Given Material A available = 500
  And Work Order cần 200 Material A
  When Planner reserve material
  Then available = 300
  And reserved = 200
  And Work Order status = MATERIAL_RESERVED
...
- Given Material A available = 100
  ...
  When Planner reserve material
  Then reserve failed
  And Work Order status = MATERIAL_SHORTAGE
  And stock không thay đổi

**TC-003 — Concurrent reservation**
- Given Material A available = 10
  When 20 request đồng thời reserve 1 Material A
  Then 10 request success
  And 10 request failed
  And available = 0
  And reserved = 10
  And stock không âm
...
- Given Work Order đang IN_PROGRESS
  When Operator complete với actual=100, good=95, defect=5, scrap=0
  Then Work Order status = COMPLETED
  And raw material bị consume
  And finished goods được tạo ở QUALITY_INSPECTION
  And QC inspection được tạo
...
- Given 95 finished goods đang QUALITY_INSPECTION
  ...
  Then 95 finished goods chuyển AVAILABLE
  And movement QC_RELEASE được tạo
